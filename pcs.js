...
function pca(X){
    ...
    let retVal;
    tf.tidy(() => {
        const xDim = X[0].length;
    const num = X.length;

    const x = tf.variable(tf.tensor2d(X, [num, xDim]));

    let xbar = tf.moments(x, 0).mean;
    // console.log("Mean");
    // xbar.print();
    ...
    retVal = [xbar.dataSync()[0], xbar.dataSync()[1], 
    xstd.dataSync()[0], xstd.dataSync()[1], 
    Zstar.dataSync()[0], Zstar.dataSync()[1]
    ];
